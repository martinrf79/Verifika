...
# LA ETAPA DE REPOSICION, en su propio modulo desde la FICHA 11: una puerta en
# vez de seis funciones sueltas. Las tres piezas que el hub sigue usando por su
# cuenta -la busqueda certificada, el producto de la memoria y el bloque de la
# cuenta- viven alla porque alla viven las seis que las usan.
...
async def procesar_venta(user_id: str, raw_message: str, tienda_id: str,
    ...
    # UNA SOLA PUERTA DESDE LA FICHA 11, en `reposicion.py`. Eran seis
    # `G.paso_datos` en fila y el orden entre ellas -que es lo unico que las
    # hace correctas- vivia en cinco comentarios de esta funcion. Las seis
    # siguen corriendo, cada una con su `G.paso_datos` adentro de la puerta;
    # lo que se corto son las cinco costuras.
    #
    # LA MEMORIA SE LE PASA SIEMPRE Y LA PUERTA DECIDE. La condicion de la
    # FICHA 04 -se abre si el reconciliador reclamo la cuenta o si el turno no
    # certifico nada- estaba suelta aca arriba, a nueve lineas de la unica
    # pieza que la usa. Ahora esta al lado.
    llamadas = G.paso_datos("reposicion", R.completar, llamadas, declarado,
                            rec, tienda_id, trace_id, memoria=_memoria_idx)
