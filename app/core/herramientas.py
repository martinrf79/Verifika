...
sola definicion. Si un molde cambia, el esquema cambia solo. Desde el
11-sep-2026 los moldes y `esquemas()` viven en `app/core/molde.py`.
"""
import json
import re
import unicodedata
...
# ── LOS MOLDES Y EL ESQUEMA QUE VE EL MODELO ─────────────────────────────────
# Viven en `app/core/molde.py` desde el 11-sep-2026. Se importan aca con sus
# nombres de siempre, asi que todo lo que dice `herramientas.RegistrarPedido`
# o `herramientas.esquemas` sigue andando sin tocarlo.
from app.core.molde import (  # noqa: F401
    Filtro, BuscarProductos, ConsultarCatalogo, FichaProducto, ConsultarTemas,
    CotizarEnvio, ItemPedido, PartePago, ArmarPresupuesto, VerCompatibilidad,
    ConsultarProductos, Cotizar, ItemDeclarado, AtributoDeclarado,
    CompatibilidadDeclarada, RegistrarPedido, CAMPOS_PEDIDO, _MOLDES,
    _aplanar, _esquema_de, _VISIBLES, esquemas, _submodelo, _sin_nulos,
)
