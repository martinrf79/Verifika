# SNAPSHOT FICHA 36, 28-ago-2026. reconciliar e instruccion_de_preguntas
# y los helpers que solo ellas usaban, copiados de app/core/pedido.py
# antes de sacarlos del vivo. No corre. app/ no importa este archivo.

from app.core.pedido import (
    _norm, _stems, _cubierto, _dos_porcentajes,
    reparto_declarado, reparto_ambiguo,
)
...
def reconciliar(pedido: dict, llamadas: list, trace_id: str = "",
    ...
    if pedido.get("pide_precio") and not any(
            n in nombres for n in ("armar_presupuesto", "cotizar")):
        if uni_prod:
