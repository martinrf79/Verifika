...
- **ABIERTO** · **LAS 15 SE REGRABARON (21-ago) Y LA HIPOTESIS ERA FALSA: no faltaban dos puntos, sobraban ocho escondidos.** Se creía que el corpus viejo castigaba al turno nuevo por no consumir una llamada de las cuatro rondas, y que regrabar devolvía 493. Regrabado con la clave gratis: **489**, con CERO huecos y CERO turnos mudos. Cuatro charlas mejoraron —`46` +3, `62` +2, `81` +2, ese era el rojo que se esperaba— pero `80_charla_real_12ago` cayó de 61/61 a **53/61**, y no es azar: **se regrabó dos veces y fallaron los MISMOS turnos 6 y 8, con el mismo motivo**, 'pidió la cuenta de un pedido concreto y el turno cerró sin Total'. El reconciliador lo reclama en el log —'todavía no armaste la cuenta'— y con DOS llamadas fijas ya no hay ronda que se lo devuelva al modelo; la reposición `_cuenta_con_lo_declarado` tampoco lo cubre porque el turno SÍ certificó productos. **El corpus viejo tapaba esto replayando respuestas grabadas cuando todavía había cuatro rondas.** El piso NO se refijó a propósito: bajarlo a 489 y subir el tope de largo de 1.882 a 2.060 sería aflojar la vara para acomodar el defecto, y el tope de largo tiene que BAJAR, no subir. Con el corpus nuevo y el piso viejo, `test_charlas_grabadas` queda en ROJO en dos tests, que es el gate haciendo su trabajo. **Lo que falta es arreglar el Total perdido en `app/`; recién ahí se refija el piso.**
