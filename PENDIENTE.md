...
- **ABIERTO** · **EL PUNTO DE OFERTA ESTA CERRADO Y EL AVANCE NO SE MOVIO: 29 de 55, igual que antes, y era lo esperable.** La FICHA 15 abrio el punto sintetico `oferta`: sobre las 15 charlas grabadas se abre en **44 de 55 turnos** y termina **OFRECIDO 10, NO_CORRESPONDE 9, SIN_ESTADO 25** — 25 turnos donde una herramienta trajo un producto que el pedido no tenia y el bot no propuso nada, casi los mismos 26 que estan rojos en avance, con las tres charlas que nunca arman pedido adentro: `62` (2), `77` (3), `79` (1). La memoria negativa ya llega al indice y se nota: dos turnos de `44_consigna_desprolijo` volvian a ofrecer lo que el cliente habia rechazado y dejaron de hacerlo. **El numero no puede moverse sin volver a grabar**: el casete reproduce el texto que el modelo escribio aquel dia, asi que la linea que la mitad 2 le pone delante al redactor no cambia una sola respuesta del corpus; `avance` sube recien cuando se regrabe. **LA SONDA DEL 25-AGO NO SE PERDIO: aparecio el contenedor que la habia grabado y se pushearon las 15 grabaciones a `banco_pruebas/casetes_sonda_25ago/`, al lado del reporte a mano de `DEFECTOS.md`, que la bateria tampoco lee.** Los numeros del reporte quedaron confirmados contra las grabaciones y ademas volvieron los instrumentos: `banco_pruebas/censo_oferta.py` mide el censo de la oferta y con `--ofertas` saca el texto entero de cada turno que ofrecio algo, y `SONDA_OFERTA_APUNTE_25ago2026.md` es el apunte de ANTES, sin el cual los numeros se mueven por el CORPUS y no se sabe si los movio el BOT. Dejo cinco defectos, ahora en `PLAN:` dentro de `tests/test_plan_de_la_sonda.py`, y **el orden no es libre: el cuarto freno de la oferta y el detector laxo van PRIMERO**, porque mientras el detector cuente como OFRECIDO cuatro frases que no ofrecen nada, regrabar es pagar la grabacion dos veces. Lo que la ficha pidio NO bajar quedo identico —`una_sola_repregunta` 54/55, el tope de largo, los otros tres puntos y todos los techos—. **Y la oferta NUNCA retiene el mensaje:** `puede_salir` la devuelve en `sin_ofrecer`, no en `omitidos`, porque una oferta que falta no se puede reponer —es prosa de venta— y un turno mudo pierde la venta entera.
