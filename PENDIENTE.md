...
- **ABIERTO** · La zona ciega del mapa son **87 funciones, no 37**: el 13-ago se arregló el instrumento —contaba la línea del `def`, que corre al IMPORTAR el módulo, como si fuera ejercicio— y aparecieron 35 que siempre estuvieron ciegas y nadie veía. Las caras son las **10 de `app/core/pago.py`**, que toca plata, y las 6 de `cierre.py`; el resto es `main.py`, los dos conectores y `llm_reintento.py`, que las 40 y los casetes solo importan. Se les escribe la prueba de a una y se baja el piso con `mapa.py --fijar`. El número no vuelve a subir: el candado rápido de `tests/test_mapa.py` no deja volver al instrumento viejo.
