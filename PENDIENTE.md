...
- **ABIERTO** · **FICHA 55 · EL RUTEO ES DEL MOTOR.** El 4.2 —una sola definicion de las bocas— cerro el 16-sep. **El 4.1 dio su primer paso el mismo dia: la planilla del pedido es obligatoria en el tablero, el cruce es por ID y lo que nadie atendio vuelve como `sin_atender`.** Falta la otra mitad: que el CODIGO elija la boca corriendo `certificar_temas`, `criterio_de`, `geo_cp` y el certificador de identidad ANTES, y no despues. **A mirar en la proxima tanda:** `renglones` y `sin_atender` en `motor_turno`. Si `renglones` sale cero, el modelo no la esta llenando y el candado del esquema no alcanzo.
