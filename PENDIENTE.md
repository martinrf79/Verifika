...
- **ABIERTO** · **LA GUARDA DE ESTADO YA MIRA, TODAVIA NO FRENA.** Nacio muda el 16-sep: cruza los campos que la respuesta nombra contra los que el turno tuvo delante —los de las condiciones y los que vinieron cargados en las fichas— y deja el renglon `afirmo_sin_mirar`, que la pelicula imprime. **Lo que falta es el numero:** una tanda por WhatsApp para ver si el renglon coincide con lo que se lee, y con eso decidir si frena y con que recorte. Hoy un campo de una palabra —`color`, `marca`— aparece en cualquier respuesta legitima, y frenar con eso adentro cambia un defecto por otro mas caro. Vara en `tests/test_guarda_de_estado.py`.
