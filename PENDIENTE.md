...
- **ABIERTO** · **LA MITAD DE LOS TURNOS NO ARMA PEDIDO: el AVANCE dio 29 de 55 y es el peor de los cinco puntos de la vara de venta.** Los 26 rojos son todos turnos que empiezan y terminan con el carrito en cero —perder un carrito ya armado no pasa una sola vez en el corpus—, y tres charlas enteras nunca llegan a tener pedido: `62_no_vendido_y_sin_dato`, `77_datos_duros` y `79_dato_falso_inducido`, donde el bot contesta la ficha tecnica bien y no ofrece cargar el producto. **Y el tercer punto, EL DETALLE NO MATA, dio pleno sobre un denominador de CUATRO turnos: es el unico numero que hoy no quiere decir nada**, porque `NO_SE_SABE` aparece en 4 puntos de 238 y la omision —`SIN_ESTADO`, 28— vive en otra bolsa. Los cinco crudos estan en `banco_pruebas/venta_piso.json` y solo suben.
