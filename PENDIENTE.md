...
- **ABIERTO** · **EL CENSO DEL CABLEADO APAREA POR NOMBRE PELADO.** `censo_cableado.funciones_sin_llamador` cruza nombres de funcion sin el modulo, asi que un `limpiar()` de otro archivo hace figurar a `huecos.limpiar` como instrumento y el censo baja solo. Destapado el 21-sep al escribir `tanda_interpretacion.py`. **Es la enfermedad de aparear por parecido que el MAPA_CABLEADO ya tiene numerada**, adentro del instrumento que mide el cableado. Arreglarlo exige resolver los imports y probablemente SUBA el censo, asi que es su propio trabajo con su propia discusion de techo.
...
- **ABIERTO** · **LA GUARDA DE ESTADO CORRIGE, Y EL PROBLEMA DE FONDO SIGUE ENTERO.** Si la respuesta afirma sobre un campo que el turno no miro, el codigo le devuelve lo que la fuente tiene y le pide de nuevo, una vez; desde el 23-sep sin tablero, porque el redactor ya no busca. Medido en vivo el 16-sep 19:46: `correcciones=1`, cuatro vueltas, y la respuesta dejo de negar el campo. **Pero el 19:47 el modelo mando otra vez cuatro consultas SIN una sola condicion:** la restriccion del cliente se sigue tirando, y esta pieza solo ataja la mentira que viene despues, no el ruteo. **Eso es la FICHA 55 §4.1 y es lo unico que lo arregla.** A mirar en la proxima tanda: `correcciones` en `motor_turno` y `correccion_de_estado` en la pelicula, para ver cuantos falsos positivos deja —`color` en un 'te lo busco por color' es uno conocido y cuesta una vuelta—.
