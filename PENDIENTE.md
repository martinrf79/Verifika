...
- **ABIERTO** · **FICHA 54 · QUEDAN DOS DE LOS CUATRO PASOS.** El 6.1 -los veinte moldes salieron de las vueltas de buscar- y el 6.3 -la contradiccion de la plata- se cerraron el 15-sep. Falta el **6.2, el doble camino de `criterio`**: `uso_recomendado` es un campo del enum del tablero y la boca de la casa se llama `criterio`, asi que el modelo filtra el catalogo en vez de preguntarle a la casa. Y falta el **6.4, el dos pasos**, que **ESPERA A MARTIN**: `compatibilidad` y `cuenta` consumen un id certificado que no existe en la primera vuelta, y hay que decidir como se pide una boca asi.
