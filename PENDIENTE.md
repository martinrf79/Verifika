...
- **ABIERTO** · **EL PUNTO DE OFERTA YA MIDE EL HUECO Y EL AVANCE NO SE MOVIO: 29 de 55, igual que antes, y era lo esperable.** La FICHA 15 abrio el punto sintetico `oferta` y `puede_salir` lo exige: sobre las 15 charlas grabadas se abre en **44 de 55 turnos** y termina **OFRECIDO 10, NO_CORRESPONDE 7, SIN_ESTADO 27** — 27 turnos donde una herramienta trajo un producto que el pedido no tenia y el bot no propuso nada, casi los mismos 26 que estaban rojos en avance, con las tres charlas que nunca arman pedido adentro: `62` (2), `77` (3), `79` (1). **El numero no puede moverse sin volver a grabar**: el casete reproduce el texto que el modelo escribio aquel dia, asi que la linea que la mitad 2 le pone delante al redactor no cambia una sola respuesta del corpus; `avance` sube recien cuando se regrabe. Lo que la ficha pidio NO bajar quedo identico —`una_sola_repregunta` 54/55, el tope de largo, los otros tres puntos y todos los techos—. **QUEDAN DOS COSAS, LAS DOS EN `hub_venta`, QUE LA FICHA TENIA PROHIBIDO TOCAR:** (1) el tercer motivo de `NO_CORRESPONDE` —"el cliente ya dijo que no lo quiere"— necesita que `descartados` llegue a `cobertura`, y hoy el indice solo recibe el carrito y lo ya mostrado; (2) `hub_venta` pega su parrafo de honestidad —"pedile al cliente lo que falte"— cada vez que `IT.instruccion` devuelve algo, y ahora eso incluye los turnos donde lo unico pendiente es ofrecer, que es un empujon justo a la segunda pregunta que la ficha prohibe.
