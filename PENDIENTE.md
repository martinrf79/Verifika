...
- **ABIERTO** · El pedido por categoría cambia de producto DENTRO del mismo turno: "2 mouse" salió 1 Genius DX-110 y 1 Logitech M170, y los auriculares 1 Negro y 1 Blanco, sin que el cliente pidiera variedad. Casete `80_charla_real_12ago` turno 7. El carrito lo arrastra bien de ahí en más; lo que falta es que una categoría con N unidades y sin criterio de variedad resuelva a UN producto.
- **ABIERTO** · `reparto_envios_detalle` de `guia_pedido` no la llama nadie en el camino vivo: solo los tests. Es la que escribe el reparto con la tarifa REAL de cada tramo. O se enchufa o se borra, pero no puede quedar viva a medias como estaba `certificar_ids_de_resultado`, que costó que con un pedido vigente no se pudiera cotizar nada nuevo.
