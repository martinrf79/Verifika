...
Sin LLM, sin credenciales, sin red. Cientos de tests sobre las piezas deterministas:
...
antijailbreak, `hub_venta`, herramientas, memoria, persistencia,
...
`piso.json` no viene en este merge a proposito: el unico piso medido
hasta ahora se saco sobre codigo que NO esta en `main`, asi que seria
una referencia falsa. La primera corrida con `--fijar-piso` sobre `main`
lo graba, y recien desde ahi la compuerta tiene contra que comparar.
Hacerlo con 5 vueltas, no con 3.
