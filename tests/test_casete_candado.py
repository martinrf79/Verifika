...
`tests/test_charlas_grabadas.py` es ahora la red principal: corre el turno
completo en cada push. Esa red tiene un modo de falla silencioso y peligroso, y
...
Estos tres candados son baratos y cubren la clase de error, no un caso puntual.
"""
import ast
...
def test_las_dos_puertas_al_modelo_existen():
    """`casete.py` parchea exactamente estos dos nombres. Si se renombran, el
    casete deja de interceptar y hay que actualizarlo en el mismo commit."""
    from app.core import generador_v2, interpretador
    assert callable(interpretador._get_client)
    assert callable(generador_v2._cliente_gemini)
...
def test_no_hay_una_tercera_puerta_al_modelo():
    ...
    """
    # Las tres puertas que el casete SI intercepta, mas el transcriptor de audio,
    ...
    permitidos = {"generador_v2.py", "llm_adapter.py", "interpretador.py",
                  "transcriber.py"}
    culpables = []
    ...
    assert not culpables, (
        ...
        + ", ".join(culpables)
        + ". Usar generador_v2._cliente_gemini() o interpretador._get_client(), "
          "o sumar la puerta nueva a banco_pruebas/casete._parchar.")
...
def test_reproducir_intercepta_de_verdad():
    ...
    casete = Casete("_prueba", [{"mensaje": "hola", "llamadas": [
        {"etapa": "solver", "salida": '{"fragmentos": []}'}]}])
    with reproducir(casete):
        casete.abrir_turno("hola")
        r = generador_v2._cliente_gemini().chat.completions.create(
            model="x", messages=[{"role": "user", "content": "hola"}],
            response_format={"type": "json_schema",
                             "json_schema": {"name": "respuesta"}})
        assert r.choices[0].message.content == '{"fragmentos": []}'
        ...
        with pytest.raises(TimeoutError):
            interpretador._get_client().chat.completions.create(
                model="x", messages=[],
                response_format={"type": "json_schema",
                                 "json_schema": {"name": "interpretacion"}})
        assert any("interprete" in f for f in casete.fallas)

    # y al salir, las puertas quedan como estaban
    assert generador_v2._cliente_gemini.__name__ == "_cliente_gemini"
    assert interpretador._get_client.__name__ == "_get_client"
